...
def view(handler, table="charts", pk="chart_id"):
    ...
    data = gdata.filter(
        variables["COARSE_LABELS"], table=table, args={pk: [handler.path_args[0]]}
    )
...
def populate_annotations():
    ...
    for _, row in df.iterrows():
        ...
        pred, mask = seg.segment_image(x, vgg, blocksize=(224, 224), plot=False)
...
def get_labels():
    data = gramex.cache.query(
        ...
        engine,
        state="SELECT COUNT(*) FROM charts",
    )
